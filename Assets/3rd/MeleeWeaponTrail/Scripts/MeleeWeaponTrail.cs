...
public class MeleeWeaponTrail : MonoBehaviour
{
    ...
    Vector3[] _tipPoints = new Vector3[4];
    Vector3[] _basePoints = new Vector3[4];
    List<Point> _remove = new List<Point>();
    List<Point> _removeCache = new List<Point>();
    List<Point> _removeCacheSmoothed = new List<Point>();
    Point _p = new Point();
    Point _sp = new Point();

    List<Vector3> _newVertices = new List<Vector3>();
    List<Vector2> _newUV = new List<Vector2>();
    ...
    void Update()
    {
        ...
        if (_emit)
        {
            if (theDistanceSqr > _minVertexDistanceSqr)
            {
                ...
                if (make)
                {
                    if (_removeCache.Count > 0)
                    {
                        _p = _removeCache[0];
                        _removeCache.RemoveAt(0);
                    }
                    else
                    {
                        _p = new Point();
                    }
                    _p.basePosition = _base.position;
                    _p.tipPosition = _tip.position;
                    _p.timeCreated = Time.time;
                    _points.Add(_p);
                    _lastPosition = transform.position;
                    ...
                    if (_points.Count >= 4)
                    {
                        ...
                        for (int n = 0; n < smoothTipList.Count; ++n)
                        {
                            ...
                            {
                                if (_removeCacheSmoothed.Count > 0)
                                {
                                    _sp = _removeCacheSmoothed[0];
                                    _removeCacheSmoothed.RemoveAt(0);
                                }
                                else
                                {
                                    _sp = new Point();
                                ...
                                _sp.basePosition = smoothBaseList[n];
                                _sp.tipPosition = smoothTipList[n];
                                _sp.timeCreated = Mathf.Lerp(firstTime, secondTime, (float)n / smoothTipList.Count);
                                _sp.isSmoothed = true;
                                _smoothedPoints[idx] = _sp;
                            }
                            //else
                            //{
        ...
        if (pointsToUse.Count > 1)
        {
            ...
            if (_newVertices.Count < pointsToUse.Count * 2)
            {
                for (int i = _newVertices.Count; i < pointsToUse.Count * 2; ++i)
                {
                    _newVertices.Add(new Vector3());
                }
            }
            Vector3[] newVertices = _newVertices.GetRange(0, pointsToUse.Count * 2).ToArray();

            if (_newUV.Count < pointsToUse.Count * 2)
            {
                for (int i = _newUV.Count; i < pointsToUse.Count * 2; ++i)
                {
                    _newUV.Add(new Vector2());
                }
            }
            Vector2[] newUV = _newUV.GetRange(0, pointsToUse.Count * 2).ToArray();
            if (_newTriangles.Count < (pointsToUse.Count - 1) * 6)
            {
                for (int i = _newTriangles.Count; i < (pointsToUse.Count - 1) * 6; ++i)
                {
                    _newTriangles.Add(new int());
                }
            }
            int[] newTriangles = _newTriangles.GetRange(0, (pointsToUse.Count - 1) * 6).ToArray();
            if (_newColors.Count < pointsToUse.Count * 2)
            {
                for (int i = _newColors.Count; i < pointsToUse.Count * 2; ++i)
                {
                    _newColors.Add(new Color());
                }
            }
            Color[] newColors = _newColors.GetRange(0, pointsToUse.Count * 2).ToArray();
    ...
    void RemoveOldPoints(List<Point> pointList)
    {
        for (int i = pointList.Count - 1; i > -1; --i)
        {
            ...
            if (Time.time - p.timeCreated > _lifeTime)
            {
                ...
                if (!_smoothedPoints.Contains(p) && !_points.Contains(p))
                {
                    if (p.isSmoothed)
                    {
                        _removeCacheSmoothed.Add(p);
                    }
                    else
                    {
                        _removeCache.Add(p);
